...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // Known-good Kapso data
  const projectId = '80697017-91d1-41c5-a37f-d502dd9f6d84';
  const phoneNumberId = '947346291806347';

  if (!KAPSO_API_KEY) {
    return res.status(500).json({ error: 'Falta la configuración de KAPSO_API_KEY en el servidor.' });
  }

  const url = `${KAPSO_API_BASE_URL}/platform/v1/projects/${projectId}/conversations?phone_number_id=${phoneNumberId}`;

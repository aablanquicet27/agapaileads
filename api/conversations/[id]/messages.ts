...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // Use the actual Kapso messages list endpoint for that conversation
  const url = `${KAPSO_API_BASE_URL}/platform/v1/projects/${projectId}/conversations/${id}/messages`;
  ...
  try {
    ...
    console.error(`Error al contactar la API de Kapso para mensajes de ${id}:`, error);
